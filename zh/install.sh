...
mkdir -p "${PROJECT_ROOT}/articles"

# --- Make scripts executable ---
echo "[INFO] Setting script permissions..."
chmod +x "${PROJECT_ROOT}/tools/extract-words.py" 2>/dev/null || true
chmod +x "${PROJECT_ROOT}/tools/count-words.py" 2>/dev/null || true
chmod +x "${PROJECT_ROOT}/tools/check-similarity.py" 2>/dev/null || true
chmod +x "${PROJECT_ROOT}/serve.sh" 2>/dev/null || true
