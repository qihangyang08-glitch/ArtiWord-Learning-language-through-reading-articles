...
- **ONLY modify `tools/word-triage-en/regex-config.txt`** — do NOT touch any other file.
...
Write ONLY the regex pattern to `tools/word-triage-en/regex-config.txt`.
...
for line, m in matches[:20]:
    ...
    word_ok = all(ord(c) < 128 or c in 'àáâãäåæçèéêëìíîïðñòóôõöøùúûüýþßÿ' for c in w)
    def_ok = len(d) >= 2
    issues = []
    if not word_ok: issues.append('word contains unexpected chars')
...
```bash
start http://localhost:8080/tools/word-triage-en/
```
